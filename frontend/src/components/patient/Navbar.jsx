...
const Navbar = ({ isDoctor }) => {
  ...
  const sendMessage = () => {
    if (message.trim()) {
      const newMessage = {
        text: message,
        sender: isDoctor ? "Doctor" : "Caretaker",
      ...
      setTimeout(() => {
        ...
        if (isDoctor) {
          ...
          if (message.toLowerCase().includes("come urgently")) {
            responseMessage = "Ok, will be there soon";
          } else {
            responseMessage = "Hi,is there anything urgent?";
          }
        }

        const autoResponse = {
          text: responseMessage,
          sender: isDoctor ? "Caretaker" : "Doctor",
          timestamp: new Date().toISOString(),
        };
  ...
  return (
    <div className="relative flex items-center justify-between whitespace-nowrap border-b border-solid border-b-[#e7eef4] px-10 py-3">
      ...
      {showChat && (
        <div className="absolute top-16 right-10 bg-white border border-gray-200 rounded-lg shadow-lg p-4 z-50 w-80">
          <h3 className="text-lg font-semibold mb-2">Chat</h3>
          <div className="h-60 overflow-y-auto mb-2">
            {messages.map((msg, index) => (
              <div key={index} className={`mb-2 ${msg.sender === (isDoctor ? "Doctor" : "Caretaker") ? "text-right" : "text-left"}`}>
